...
- **Unexplained mid-task drift.** A dependency added mid-phase with no brief support, TODO/FIXME left in code, "temporary" constants. The rationale is in your head right now; it won't be in PR review. A new third-party dependency is the sharp case: that a library is the *obvious* or *standard* tool for the task (a mocking framework to write a test, an HTTP client for one call) is not the brief asking for it. Pulling a dependency into the project is a decision the orchestrator owns; "it's the normal way to do this" is the rationalization to catch, not the license to proceed. A hand-rolled fake or a few lines of stdlib usually costs less than the dependency you were about to add.
